...
*/
class AST {
    constructor(roots, compilerVersion) {
        this.roots = roots;
        ...
        (0, assert_1.default)(roots.every((sourceUnit) => sourceUnit.requiredContext === roots[0].requiredContext), 'All contexts should be the same, otherwise they are from seperate solc-typed-ast compiles and they will have no relationship to each other.');
    ...
    copyRegisteredImports(oldNode, newNode) {
        this.imports.set(newNode, (0, utils_1.mergeImports)(this.imports.get(oldNode) ?? new Map(), this.imports.get(newNode) ?? new Map()));
    }
    extractToConstant(node, vType, newName) {
        ...
        if (node instanceof solc_typed_ast_1.FunctionCall && (0, utils_1.isExternalCall)(node)) {
            location =
                (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(node, this.compilerVersion))[1] === undefined
                    ? solc_typed_ast_1.DataLocation.Default
                    : solc_typed_ast_1.DataLocation.CallData;
        }
        else {
            location = (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(node, this.compilerVersion))[1] ?? solc_typed_ast_1.DataLocation.Default;
        }
        const replacementVariable = new solc_typed_ast_1.VariableDeclaration(this.tempId, node.src, true, false, newName, scope, false, location, solc_typed_ast_1.StateVariableVisibility.Private, solc_typed_ast_1.Mutability.Constant, node.typeString, undefined, vType);
        ...
        const replacementIdentifer = new solc_typed_ast_1.Identifier(this.tempId, node.src, node.typeString, newName, replacementVariable.id, node.raw);
    ...
    getContainingRoot(node) {
        ...
        (0, assert_1.default)(root !== undefined, `Could not find root source unit for ${(0, astPrinter_1.printNode)(node)}`);
    ...
    getImplicitsAt(node) {
        const containingFunction = node.getClosestParentByType(cairoNodes_1.CairoFunctionDefinition);
        if (containingFunction === undefined)
            return new Set();
        return containingFunction.implicits;
    }
    getImports(sourceUnit) {
        (0, assert_1.default)(this.roots.includes(sourceUnit), `Tried to get imports associated with ${(0, astPrinter_1.printNode)(sourceUnit)}, which is not one of the roots of the AST`);
        const reachableNodeImports = sourceUnit
            .getChildren(true)
            .map((node) => this.imports.get(node) ?? new Map());
        const utilFunctionImports = this.getUtilFuncGen(sourceUnit)?.getImports();
        return (0, utils_1.mergeImports)(utilFunctionImports, ...reachableNodeImports);
    }
    getUtilFuncGen(node) {
        const sourceUnit = node instanceof solc_typed_ast_1.SourceUnit ? node : node.getClosestParentByType(solc_typed_ast_1.SourceUnit);
        (0, assert_1.default)(sourceUnit !== undefined, 'Could not find the sourceUnit to attach the nodes generated functions to');
    ...
    registerImport(node, location, name) {
        const nodeImports = this.imports.get(node) ?? new Map();
        const fileImports = nodeImports.get(location) ?? new Set();
    ...
    replaceNode(oldNode, newNode, parent, copyImports = true) {
        if (oldNode === newNode) {
