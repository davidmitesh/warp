...
function checkVFieldCtx(node, prop, ctx) {
    ...
    if (val instanceof solc_typed_ast_1.ASTNode) {
        if (!inCtx(val, ctx)) {
            throw new Error(`Node ${(0, index_1.pp)(node)} property ${prop} ${(0, index_1.pp)(val)} not in expected context ${(0, index_1.pp)(ctx)}. Instead in ${(0, index_1.pp)(val.context)}`);
        }
    }
        ...
        for (let idx = 0; idx < val.length; idx++) {
            ...
            if (!(el instanceof solc_typed_ast_1.ASTNode)) {
                throw new Error(`Expected property ${prop}[${idx}] of ${(0, index_1.pp)(node)} to be an ASTNode not ${el}`);
            }
            if (!inCtx(val, ctx)) {
                throw new Error(`Node ${(0, index_1.pp)(node)} property ${prop}[${idx}] ${(0, index_1.pp)(el)} not in expected context ${(0, index_1.pp)(ctx)}. Instead in ${(0, index_1.pp)(el.context)}`);
            }
        }
    }
    else {
        throw new Error(`Expected property ${prop} of ${(0, index_1.pp)(node)} to be an ASTNode, not ${val}`);
    }
}
/**
...
function checkFieldAndVFieldMatch(node, field, vField) {
    ...
    if (typeof val1 === 'number') {
        if (!(val2 instanceof solc_typed_ast_1.ASTNode)) {
            throw new Error(`Expected property ${vField} of ${(0, index_1.pp)(node)} to be an ASTNode when ${field} is a number, not ${val2}`);
        }
        if (val1 != val2.id) {
            throw new solc_typed_ast_1.InsaneASTError(`Node ${(0, index_1.pp)(node)} property ${field} ${val1} differs from ${vField}.id ${(0, index_1.pp)(val2)}`);
        }
    }
    else if (val1 instanceof Array) {
        if (!(val2 instanceof Array)) {
            throw new Error(`Expected property ${vField} of ${(0, index_1.pp)(node)} to be an array when ${vField} is an array, not ${val2}`);
        }
        if (val1.length !== val2.length) {
            throw new solc_typed_ast_1.InsaneASTError(`Node ${(0, index_1.pp)(node)} array properties ${field} and ${vField} have different lengths ${val1.length} != ${val2.length}`);
        }
        for (let idx = 0; idx < val1.length; idx++) {
            const el1 = val1[idx];
            const el2 = val2[idx];
            if (typeof el1 !== 'number') {
                throw new Error(`Expected property ${field}[${idx}] of ${(0, index_1.pp)(node)} to be a number not ${el1}`);
            }
            if (!(el2 instanceof solc_typed_ast_1.ASTNode)) {
                throw new Error(`Expected property ${vField}[${idx}] of ${(0, index_1.pp)(node)} to be a number not ${el2}`);
            }
            if (el1 != el2.id) {
                throw new solc_typed_ast_1.InsaneASTError(`Node ${(0, index_1.pp)(node)} property ${field}[${idx}] ${el1} differs from ${vField}[${idx}].id ${(0, index_1.pp)(el2)}`);
            }
        }
    }
    else {
        throw new Error(`Expected property ${field} of ${(0, index_1.pp)(node)} to be a number or  array of numbers not ${val1}`);
    }
}
/**
 * Helper to check that:
...
function checkDirectChildren(node, ...fields) {
    ...
    for (const field of fields) {
        ...
        if (val instanceof solc_typed_ast_1.ASTNode) {
            if (!directChildren.has(val)) {
                throw new solc_typed_ast_1.InsaneASTError(`Field ${field} of node ${(0, index_1.pp)(node)} is not a direct child: ${(0, index_1.pp)(val)} child of ${(0, index_1.pp)(val.parent)}`);
            }
            ...
            for (let i = 0; i < val.length; i++) {
                ...
                if (!(el instanceof solc_typed_ast_1.ASTNode)) {
                    throw new Error(`Field ${field} of ${(0, index_1.pp)(node)} is expected to be ASTNode, array or map with ASTNodes - instead array containing ${el}`);
                }
                if (!directChildren.has(el)) {
                    throw new solc_typed_ast_1.InsaneASTError(`Field ${field}[${i}] of node ${(0, index_1.pp)(node)} is not a direct child: ${(0, index_1.pp)(el)} child of ${(0, index_1.pp)(el.parent)}`);
                }
            ...
            for (const [k, v] of val.entries()) {
                ...
                if (!(v instanceof solc_typed_ast_1.ASTNode)) {
                    throw new Error(`Field ${field} of ${(0, index_1.pp)(node)} is expected to be ASTNode, array or map with ASTNodes - instead map containing ${v}`);
                }
                if (!directChildren.has(v)) {
                    throw new solc_typed_ast_1.InsaneASTError(`Field ${field}[${k}] of node ${(0, index_1.pp)(node)} is not a direct child: ${(0, index_1.pp)(v)} child of ${(0, index_1.pp)(v.parent)}`);
                }
                computedChildren.add(v);
            }
        }
        else {
            throw new Error(`Field ${field} of ${(0, index_1.pp)(node)} is neither an ASTNode nor an array of ASTNode or nulls: ${val}`);
        }
    }
    if (computedChildren.size < directChildren.size) {
        ...
        throw new solc_typed_ast_1.InsaneASTError(`Fields ${fields.join(', ')} don't completely cover the direct children: ${(0, index_1.pp)(missingChild)} is missing`);
    }
}
/**
...
function checkSane(unit, ctx) {
    for (const node of unit.getChildren(true)) {
        if (!inCtx(node, ctx)) {
            throw new solc_typed_ast_1.InsaneASTError(`Child ${(0, index_1.pp)(node)} in different context: ${ctx.id} from expected ${ctx.id}`);
        }
        const immediateChildren = node.children;
        for (const child of immediateChildren) {
            if (child.parent !== node) {
                throw new solc_typed_ast_1.InsaneASTError(`Child ${(0, index_1.pp)(child)} has wrong parent: expected ${(0, index_1.pp)(node)} got ${(0, index_1.pp)(child.parent)}`);
            }
        }
        ...
        if (node instanceof solc_typed_ast_1.SourceUnit) {
            for (const [name, symId] of node.exportedSymbols) {
                ...
                if (symNode === undefined) {
                    throw new solc_typed_ast_1.InsaneASTError(`Exported symbol ${name} ${symId} missing from context ${ctx.id}`);
                }
                if (symNode !== node.vExportedSymbols.get(name)) {
                    throw new solc_typed_ast_1.InsaneASTError(`Exported symbol ${name} for id ${symId} (${(0, index_1.pp)(symNode)}) doesn't match vExportedSymbols entry: ${(0, index_1.pp)(node.vExportedSymbols.get(name))}`);
                }
            }
            ...
            if (node.vSymbolAliases.length !== 0 &&
                ...
                throw new solc_typed_ast_1.InsaneASTError(`symbolAliases.length (${node.symbolAliases.length}) and vSymboliAliases.length ${node.vSymbolAliases.length} misamtch for import ${(0, index_1.pp)(node)}`);
            }
            for (let i = 0; i < node.vSymbolAliases.length; i++) {
                const def = node.vSymbolAliases[i][0];
                if (!inCtx(def, ctx)) {
                    throw new solc_typed_ast_1.InsaneASTError(`Imported symbol ${(0, index_1.pp)(def)} from import ${(0, index_1.pp)(node)} not in expected context ${(0, index_1.pp)(ctx)}`);
                }
            }
            ...
            if (node.vScope !== node.parent) {
                throw new solc_typed_ast_1.InsaneASTError(`Contract ${(0, index_1.pp)(node)} vScope ${(0, index_1.pp)(node.vScope)} and parent ${(0, index_1.pp)(node.parent)} differ`);
            }
...
function checkContextsDefined(node) {
    ...
    if (context === undefined) {
        throw new solc_typed_ast_1.InsaneASTError(`${node.type} ${node.id} has no context`);
    }
    node.children.forEach((child) => checkContextsDefined(child));
}
function checkIdNonNegative(node) {
    if (node.id < 0)
        throw new solc_typed_ast_1.InsaneASTError(`${node.type} ${node.id} has invalid id`);
...
function checkOnlyConstructorsMarkedAsConstructors(nodes) {
    nodes.forEach((func) => {
        if ((func.kind === solc_typed_ast_1.FunctionKind.Constructor) !== func.isConstructor)
            throw new solc_typed_ast_1.InsaneASTError(`${(0, astPrinter_1.printNode)(func)} ${func.name} is incorrectly marked as ${func.isConstructor ? '' : 'not '} a constructor`);
    });
}
...
function checkEmptyNamesForNamelessFunctions(nodes) {
    nodes.forEach((func) => {
        if (func.name === '' && !(0, utils_1.isNameless)(func)) {
            throw new solc_typed_ast_1.InsaneASTError(`${(0, astPrinter_1.printNode)(func)} has an empty name but it's kind: ${func.kind} is different than constructor, fallback or receive`);
        }
        if (func.name !== '' && (0, utils_1.isNameless)(func)) {
            throw new solc_typed_ast_1.InsaneASTError(`${(0, astPrinter_1.printNode)(func)} of kind ${func.kind} has a non-empty name: ${func.name}`);
        }
    });
}
/**
...
function isSane(ast) {
    ...
    return ast.roots.every((root) => {
        try {
            ...
            if (e instanceof solc_typed_ast_1.InsaneASTError) {
                console.error(e);
                return false;
            }
            throw e;
...
class NodeTypeResolutionChecker extends mapper_1.ASTMapper {
    visitSourceUnit(node, ast) {
        node
            ...
            .forEach((child) => (0, solc_typed_ast_1.getNodeType)(child, ast.compilerVersion));
    }
}
    ...
    visitFunctionDefinition(node, _ast) {
        [...node.vParameters.vParameters, ...node.vReturnParameters.vParameters].forEach((decl) => {
            if (decl.scope !== node.id) {
                throw new solc_typed_ast_1.InsaneASTError(`${(0, astPrinter_1.printNode)(decl)} in ${(0, astPrinter_1.printNode)(node)} has scope ${decl.scope}, expected ${node.id}`);
            }
        });
    }
}
