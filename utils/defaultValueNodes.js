...
function arrayDefault(nodeType, parentNode, ast) {
    ...
    if (nodeType.size === undefined) {
        ...
        return new solc_typed_ast_1.TupleExpression(ast.reserveId(), parentNode.src, `${getTupleTypeString(nodeType)} memory`, true, expList, parentNode.raw);
    }
}
...
function userDefDefault(nodeType, parentNode, ast) {
    ...
    if (nodeType.definition instanceof solc_typed_ast_1.UserDefinedValueTypeDefinition)
        return getDefaultValue((0, solc_typed_ast_1.getNodeType)(nodeType.definition.underlyingType, ast.compilerVersion), parentNode, ast);
    throw new errors_1.TranspileFailedError(`Not supported operation delete on ${(0, astPrinter_1.printNode)(nodeType.definition)}`);
}
...
function structDefault(structNode, node, ast) {
    ...
    for (const member of structNode.vMembers) {
        const tNode = (0, solc_typed_ast_1.getNodeType)(member, ast.compilerVersion);
...
function pointerDefault(nodeType, parentNode, ast) {
    if (nodeType.to instanceof solc_typed_ast_1.ArrayType)
        ...
        throw new errors_1.NotSupportedYetError(`Not supported operation delete on ${(0, astPrinter_1.printTypeNode)(nodeType)}`);
    }
}
function getTupleTypeString(nodeType) {
    ...
    if (node instanceof solc_typed_ast_1.PointerType) {
        if (node.to instanceof solc_typed_ast_1.ArrayType)
            ...
            throw new errors_1.NotSupportedYetError(`Not supported operation delete on ${(0, astPrinter_1.printTypeNode)(node.to)}`);
    }
    else {
        return nodeType.pp();
    }
