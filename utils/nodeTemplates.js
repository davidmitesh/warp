...
exports.createVariableDeclarationStatement = exports.createDefaultConstructor = exports.createUintNTypeName = exports.createUint8TypeName = exports.createUint256TypeName = exports.createReturn = exports.createParameterList = exports.createStringLiteral = exports.createNumberTypeName = exports.createNumberLiteral = exports.createIdentifier = exports.createExpressionStatement = exports.createEmptyTuple = exports.createBytesNTypeName = exports.createBytesTypeName = exports.createBoolTypeName = exports.createBoolLiteral = exports.createBlock = exports.createArrayTypeName = exports.createStringTypeName = exports.createAddressTypeName = void 0;
...
function createStringTypeName(payable, ast) {
    const node = new solc_typed_ast_1.ElementaryTypeName(ast.reserveId(), '', 'string', 'string', payable ? 'payable' : 'nonpayable');
    ast.setContextRecursive(node);
    return node;
}
...
function createEmptyTuple(ast) {
    const node = new solc_typed_ast_1.TupleExpression(ast.reserveId(), '', 'tuple()', false, []);
    ast.setContextRecursive(node);
    return node;
}
...
function createIdentifier(variable, ast, dataLocation, lookupNode) {
    const type = (0, nodeTypeProcessing_1.specializeType)((0, solc_typed_ast_1.getNodeTypeInCtx)(variable, ast.compilerVersion, lookupNode ?? variable), dataLocation ?? (variable.stateVariable ? solc_typed_ast_1.DataLocation.Storage : variable.storageLocation));
...
function createDefaultConstructor(node, ast) {
    const newFunc = new solc_typed_ast_1.FunctionDefinition(ast.reserveId(), '', node.id, solc_typed_ast_1.FunctionKind.Constructor, '', false, solc_typed_ast_1.FunctionVisibility.Public, solc_typed_ast_1.FunctionStateMutability.NonPayable, true, createParameterList([], ast), createParameterList([], ast), []);
    ast.setContextRecursive(newFunc);
    return newFunc;
}
exports.createDefaultConstructor = createDefaultConstructor;
function createVariableDeclarationStatement(varDecls, intitalValue, ast) {
    ...
    const node = new solc_typed_ast_1.VariableDeclarationStatement(ast.reserveId(), '', varDecls.map((v) => (v === null ? null : v.id)), varDecls.filter(typeConstructs_1.notNull), intitalValue);
    ast.setContextRecursive(node);
    return node;
}
