...
class BytesConverter extends mapper_1.ASTMapper {
    ...
    visitExpression(node, ast) {
        ...
        if (!(typeNode instanceof solc_typed_ast_1.IntLiteralType)) {
    ...
    visitVariableDeclaration(node, ast) {
        const typeNode = replaceBytesType((0, solc_typed_ast_1.getNodeType)(node, ast.compilerVersion));
        node.typeString = (0, getTypeString_1.generateExpressionTypeString)(typeNode);
        this.commonVisit(node, ast);
    }
    visitElementaryTypeName(node, ast) {
        const typeNode = (0, solc_typed_ast_1.typeNameToTypeNode)(node);
    ...
    visitIndexAccess(node, ast) {
        if (!(node.vIndexExpression &&
            (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(node.vBaseExpression, ast.compilerVersion))[0] instanceof
                solc_typed_ast_1.FixedBytesType)) {
            this.visitExpression(node, ast);
            return;
        }
        const baseTypeName = (0, utils_1.typeNameFromTypeNode)((0, solc_typed_ast_1.getNodeType)(node.vBaseExpression, ast.compilerVersion), ast);
        ...
        const indexTypeName = node.vIndexExpression instanceof solc_typed_ast_1.Literal
            ...
            : (0, utils_1.typeNameFromTypeNode)((0, solc_typed_ast_1.getNodeType)(node.vIndexExpression, ast.compilerVersion), ast);
        const stubParams = [
            ['base', baseTypeName],
            ['index', indexTypeName],
        ];
        ...
        const functionStub = (0, functionGeneration_1.createCairoFunctionStub)(selectWarplibFunction(baseTypeName, indexTypeName), stubParams, [['res', (0, nodeTemplates_1.createUint8TypeName)(ast)]], ['bitwise_ptr', 'range_check_ptr'], ast, node);
        const call = (0, functionGeneration_1.createCallToFunction)(functionStub, callArgs, ast);
        ast.registerImport(call, 'warplib.maths.bytes_access', selectWarplibFunction(baseTypeName, indexTypeName));
        ast.replaceNode(node, call, node.parent);
        const typeNode = replaceBytesType((0, solc_typed_ast_1.getNodeType)(call, ast.compilerVersion));
...
function replaceBytesType(type) {
    if (type instanceof solc_typed_ast_1.ArrayType) {
        ...
        return new solc_typed_ast_1.FunctionType(type.name, type.parameters.map(replaceBytesType), type.returns.map(replaceBytesType), type.visibility, type.mutability, type.src);
    }
