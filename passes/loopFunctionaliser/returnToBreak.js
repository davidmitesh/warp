...
class ReturnToBreak extends mapper_1.ASTMapper {
    ...
    createReturnFlag(containingWhile, ast) {
        const decl = new solc_typed_ast_1.VariableDeclaration(ast.reserveId(), '', false, false, `${nameModifiers_1.RETURN_FLAG_PREFIX}${this.returnFlags.size}`, ast.getContainingScope(containingWhile), false, solc_typed_ast_1.DataLocation.Default, solc_typed_ast_1.StateVariableVisibility.Default, solc_typed_ast_1.Mutability.Mutable, 'bool', undefined, (0, nodeTemplates_1.createBoolTypeName)(ast));
    ...
    getReturnVariables(node) {
        ...
        (0, assert_1.default)(containingFunction !== undefined, `Could not find a containing function for ${(0, astPrinter_1.printNode)(node)}`);
...
function insertReturnValueDeclaration(node, ast) {
    const containingFunction = node.getClosestParentByType(solc_typed_ast_1.FunctionDefinition);
    (0, assert_1.default)(containingFunction !== undefined, `Unable to find containing function for ${node}`);
