...
class FunctionModifierHandler extends mapper_1.ASTMapper {
    ...
    getFunctionFromModifier(node, modifier, functionToCall, ast) {
        ...
        const modifierAsFunction = new solc_typed_ast_1.FunctionDefinition(modifierClone.id, modifierClone.src, node.scope, solc_typed_ast_1.FunctionKind.Function, `__warp_${modifier.name}_${this.count++}`, node.virtual, solc_typed_ast_1.FunctionVisibility.Internal, node.stateMutability, false, (0, nodeTemplates_1.createParameterList)([...modifierClone.vParameters.vParameters, ...functionParams], ast, modifierClone.id), retParamList, [], undefined, modifierClone.vBody);
    ...
    createInputParameter(v, ast) {
        ...
        variable.name = `${nameModifiers_1.MANGLED_PARAMETER}${this.count++}`;
        return variable;
    }
    createReturnParameter(v, ast) {
        const param = (0, cloning_1.cloneASTNode)(v, ast);
        param.name = `${nameModifiers_1.MANGLED_RETURN_PARAMETER}${this.count++}`;
        return param;
    }
}
