...
class ImplicitConversionToExplicit extends mapper_1.ASTMapper {
    ...
    visitReturn(node, ast) {
        ...
        if (returnDeclarations.length !== 1)
            return;
        const expectedRetType = (0, solc_typed_ast_1.getNodeType)(returnDeclarations[0], ast.compilerVersion);
        insertConversionIfNecessary(node.vExpression, expectedRetType, ast);
    }
    visitBinaryOperation(node, ast) {
        this.commonVisit(node, ast);
        const resultType = (0, solc_typed_ast_1.getNodeType)(node, ast.compilerVersion);
        if (node.operator === '<<' || node.operator === '>>') {
            ...
            if (rhsType instanceof solc_typed_ast_1.IntLiteralType) {
                insertConversionIfNecessary(node.vRightExpression, (0, nodeTypeProcessing_1.intTypeForLiteral)(node.vRightExpression.typeString), ast);
            }
            return;
        }
        else if (node.operator === '**') {
            const rightNodeType = (0, solc_typed_ast_1.getNodeType)(node.vRightExpression, ast.compilerVersion);
            insertConversionIfNecessary(node.vLeftExpression, resultType, ast);
            if (rightNodeType instanceof solc_typed_ast_1.IntLiteralType) {
                ...
                insertConversionIfNecessary(node.vRightExpression, (0, nodeTypeProcessing_1.intTypeForLiteral)(`int_const ${bound}`), ast);
            }
        }
            ...
            const leftNodeType = (0, solc_typed_ast_1.getNodeType)(node.vLeftExpression, ast.compilerVersion);
            const rightNodeType = (0, solc_typed_ast_1.getNodeType)(node.vRightExpression, ast.compilerVersion);
    ...
    visitAssignment(node, ast) {
        this.commonVisit(node, ast);
        insertConversionIfNecessary(node.vRightHandSide, (0, solc_typed_ast_1.getNodeType)(node.vLeftHandSide, ast.compilerVersion), ast);
    }
    visitVariableDeclarationStatement(node, ast) {
        this.commonVisit(node, ast);
        (0, assert_1.default)(node.vInitialValue !== undefined, `Implicit conversion to explicit expects variables to be initialised (did you run variable declaration initialiser?). Found at ${(0, astPrinter_1.printNode)(node)}`);
        ...
        if (node.assignments.length !== 1)
            return;
        insertConversionIfNecessary(node.vInitialValue, (0, solc_typed_ast_1.getNodeType)(node.vDeclarations[0], ast.compilerVersion), ast);
    }
    visitFunctionCall(node, ast) {
        ...
        const paramTypes = (0, nodeTypeProcessing_1.getParameterTypes)(node, ast);
        (0, assert_1.default)(paramTypes.length === node.vArguments.length, (0, formatting_1.error)(`${(0, astPrinter_1.printNode)(node)} has incorrect number of arguments. Expected ${paramTypes.length}, got ${node.vArguments.length}`));
        node.vArguments.forEach((arg, index) => insertConversionIfNecessary(arg, paramTypes[index], ast));
    }
    visitIndexAccess(node, ast) {
        this.commonVisit(node, ast);
        if (node.vIndexExpression === undefined)
            return;
        const [baseType, location] = (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(node.vBaseExpression, ast.compilerVersion));
        if (baseType instanceof solc_typed_ast_1.MappingType) {
            ...
            insertConversionIfNecessary(node.vIndexExpression, new solc_typed_ast_1.IntType(248, false), ast);
        }
        else {
            insertConversionIfNecessary(node.vIndexExpression, new solc_typed_ast_1.IntType(256, false), ast);
        }
    }
...
function insertConversionIfNecessary(expression, targetType, ast) {
    const [currentType, currentLoc] = (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(expression, ast.compilerVersion));
    ...
    else if (currentType instanceof solc_typed_ast_1.ModuleType) {
        return;
    }
...
function insertConversion(expression, targetType, ast) {
    const typeName = (0, utils_1.typeNameFromTypeNode)(targetType, ast);
    (0, assert_1.default)(typeName instanceof solc_typed_ast_1.ElementaryTypeName, `Attempted elementary conversion to non-elementary type: ${(0, solc_typed_ast_1.getNodeType)(expression, ast.compilerVersion).pp()} -> ${(0, astPrinter_1.printTypeNode)(targetType)}`);
    const parent = expression.parent;
    const call = (0, functionGeneration_1.createElementaryConversionCall)(typeName, expression, ast);
...
function pickLargerType(typeA, typeB, leftLiteralBound, rightLiteralBound) {
    ...
    if (typeA instanceof solc_typed_ast_1.StringLiteralType) {
        if (typeB instanceof solc_typed_ast_1.StringLiteralType) {
            const length = Math.max(typeA.literal.length, typeB.literal.length);
            if (length >= 32) {
                return new solc_typed_ast_1.BytesType();
            }
            return new solc_typed_ast_1.FixedBytesType(length);
        }
        else {
            return typeB;
        }
...
function handleConcatArgs(node, ast) {
    node.vArguments.forEach((arg) => {
        const type = (0, solc_typed_ast_1.getNodeType)(arg, ast.compilerVersion);
        if (type instanceof solc_typed_ast_1.StringLiteralType) {
            if (type.literal.length < 32 && type.literal.length > 0) {
                insertConversionIfNecessary(arg, new solc_typed_ast_1.FixedBytesType(type.literal.length), ast);
            }
            else {
                insertConversionIfNecessary(arg, new solc_typed_ast_1.BytesType(), ast);
            }
        }
    });
}
