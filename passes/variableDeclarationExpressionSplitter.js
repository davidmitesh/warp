...
class VariableDeclarationExpressionSplitter extends mapper_1.ASTMapper {
    ...
    splitDeclaration(node, ast) {
        ...
        const initialValueType = (0, solc_typed_ast_1.getNodeType)(initialValue, ast.compilerVersion);
        ...
        if (initialValue instanceof solc_typed_ast_1.FunctionCall) {
            ...
            const newAssignedIds = node.assignments.map((id, index) => {
                ...
                if (oldDeclaration.typeString === initialValueType.elements[index].pp()) {
                    ...
                    const newDeclaration = new solc_typed_ast_1.VariableDeclaration(ast.reserveId(), node.src, true, false, this.generateNewConstantName(), oldDeclaration.scope, false, oldDeclaration.storageLocation, solc_typed_ast_1.StateVariableVisibility.Default, solc_typed_ast_1.Mutability.Constant, newTypeName.typeString, undefined, newTypeName);
