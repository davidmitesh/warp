            ...
            if (actualLoc === solc_typed_ast_1.DataLocation.Storage) {
                switch (expectedLoc) {
                    case solc_typed_ast_1.DataLocation.Default: {
                        copyFunc = utilFuncGen.storage.read.gen(node, (0, utils_1.typeNameFromTypeNode)(nodeType, ast));
                        break;
                    }
                ...
                switch (expectedLoc) {
                    case solc_typed_ast_1.DataLocation.Default: {
                        copyFunc = utilFuncGen.memory.read.gen(node, (0, utils_1.typeNameFromTypeNode)(nodeType, ast));
                        break;
                    }
        ...
        if (toLoc === solc_typed_ast_1.DataLocation.Memory) {
            ...
            if (fromLoc === solc_typed_ast_1.DataLocation.Storage) {
                ...
                const [convert, result] = ast
                    .getUtilFuncGen(node)
                    .memory.convert.genIfNecesary(node.vLeftHandSide, (0, solc_typed_ast_1.getNodeType)(node.vRightHandSide, ast.compilerVersion));
                if (result) {
    ...
    visitMemberAccess(node, ast) {
        ...
        if (!(type instanceof solc_typed_ast_1.PointerType)) {
            (0, assert_1.default)((type instanceof solc_typed_ast_1.UserDefinedType && type.definition instanceof solc_typed_ast_1.ContractDefinition) ||
                type instanceof solc_typed_ast_1.FixedBytesType, `Unexpected unhandled non-pointer non-contract member access. Found at ${(0, astPrinter_1.printNode)(node)}`);
            return this.visitExpression(node, ast);
        }
        const utilFuncGen = ast.getUtilFuncGen(node);
...
function createMemoryDynArrayIndexAccess(indexAccess, ast) {
    const arrayType = (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(indexAccess.vBaseExpression, ast.compilerVersion))[0];
    ...
    const stub = (0, functionGeneration_1.createCairoFunctionStub)('wm_index_dyn', [
    ...
    ], [['loc', returnTypeName, solc_typed_ast_1.DataLocation.Memory]], ['range_check_ptr', 'warp_memory'], ast, indexAccess);
    ...
    const call = (0, functionGeneration_1.createCallToFunction)(stub, [
