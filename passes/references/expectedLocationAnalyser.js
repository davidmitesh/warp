...
class ExpectedLocationAnalyser extends mapper_1.ASTMapper {
    ...
    visitAssignment(node, ast) {
        ...
        if (lhsLocation === solc_typed_ast_1.DataLocation.Storage) {
            ...
            const rhsLocation = (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(node.vRightHandSide, ast.compilerVersion))[1] ??
            ...
            this.expectedLocations.set(node.vLeftHandSide, lhsLocation);
            const rhsType = (0, solc_typed_ast_1.getNodeType)(node.vRightHandSide, ast.compilerVersion);
    ...
    visitFunctionCall(node, ast) {
        if (node.kind === solc_typed_ast_1.FunctionCallKind.TypeConversion) {
            const toType = (0, solc_typed_ast_1.getNodeType)(node, ast.compilerVersion);
            node.vArguments.forEach((arg) => {
                const [type, location] = (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(arg, ast.compilerVersion));
        ...
        node.vArguments.forEach((arg, index) => {
            ...
            if (t instanceof solc_typed_ast_1.PointerType) {
                if (node.kind === solc_typed_ast_1.FunctionCallKind.StructConstructorCall) {
                    ...
                    const structType = (0, solc_typed_ast_1.getNodeType)(node, ast.compilerVersion);
    ...
    visitIndexAccess(node, ast) {
        ...
        if (baseType instanceof solc_typed_ast_1.PointerType &&
            ...
            const indexLoc = (0, solc_typed_ast_1.generalizeType)((0, solc_typed_ast_1.getNodeType)(node.vIndexExpression, ast.compilerVersion))[1];
    ...
    visitMemberAccess(node, ast) {
        ...
        const baseNodeType = (0, solc_typed_ast_1.getNodeType)(node.vExpression, ast.compilerVersion);
        (0, assert_1.default)(baseLoc !== undefined);
        ...
        if ((0, utils_1.isExternallyVisible)(func)) {
            if (node.vExpression) {
                ...
                retExpressions.forEach((retExpression) => {
                    const retType = (0, solc_typed_ast_1.getNodeType)(retExpression, ast.compilerVersion);
    ...
    visitTupleExpression(node, ast) {
        ...
        node.vOriginalComponents.filter(typeConstructs_1.notNull).forEach((element) => {
            const elementType = (0, solc_typed_ast_1.getNodeType)(element, ast.compilerVersion);
