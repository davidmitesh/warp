...
class ThisKeyword extends mapper_1.ASTMapper {
    visitIdentifier(node, ast) {
        if (node.name === 'this') {
            const replacementCall = (0, functionGeneration_1.createCallToFunction)((0, functionGeneration_1.createCairoFunctionStub)('get_contract_address', [], [['address', (0, utils_1.typeNameFromTypeNode)((0, solc_typed_ast_1.getNodeType)(node, ast.compilerVersion), ast)]], ['syscall_ptr'], ast, node), [], ast);
            ast.replaceNode(node, replacementCall);
            ast.registerImport(replacementCall, 'starkware.starknet.common.syscalls', 'get_contract_address');
        }
        else {
            return;
        }
