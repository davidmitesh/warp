...
class Require extends mapper_1.ASTMapper {
    ...
    requireToCairoAssert(expression, ast) {
        ...
        if (expression.vIdentifier === 'require' || expression.vIdentifier === 'assert') {
            if (expression.vArguments.length > 1 && !(expression.vArguments[1] instanceof solc_typed_ast_1.Literal)) {
                throw new errors_1.WillNotSupportError(`Error messages must be string literals for assert, revert, and require`);
            }
            ...
            if (expression.vArguments.length > 0 && !(expression.vArguments[0] instanceof solc_typed_ast_1.Literal)) {
                throw new errors_1.WillNotSupportError(`Error messages must be string literals for assert, revert, and require`);
            }
