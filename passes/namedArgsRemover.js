    ...
    visitFunctionCall(node, ast) {
        ...
        */
        ...
        if (orderedFieldNames.length !== node.vArguments.length ||
            ...
            throw new errors_1.TranspileFailedError('Number of arguments must less or euqal to number of function field names');
        }
        orderedFieldNames.forEach((fieldName, index) => {
            ...
            if (fieldNameIndex === -1) {
                throw new errors_1.TranspileFailedError(`Field name not found: ${fieldName} in ${node.vReferencedDeclaration?.constructor.name}`);
            }
